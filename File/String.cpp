...
const String String::toUppercase() const
{
	...
	char* uppercase = new char[this->length() + 1];
	...
	String uppercasedStr(uppercase);
...
{
	// Create a char array to store the uppercase letters
	char* lowercase = new char[this->length() + 1];
	...
	{
		// Takes the uppercased character and adds it to the array
		lowercase[c - this->c_str] = std::tolower(*c);

		++c;
	}

	// Manually append null terminating byte
	lowercase[this->length()] = '\0';
	String lowercasedStr(lowercase);
